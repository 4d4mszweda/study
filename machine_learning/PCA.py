...
def pca_visualization():
    ...
    print("Liczba komponentów potrzebna do zachowania 95% wariancji: ", num_components)

    for i, variance in enumerate(pca.explained_variance_ratio_):
        print(f"Główna składowa {i+1}: {variance:.2%} wariancji")

    # Redukcja danych do odpowiedniej liczby wymiarów
    pca_reduced = pca_fit[:, :num_components]

    if num_components == 2:
        plt.figure(figsize=(8,6))
        plt.scatter(pca_reduced[:, 0], pca_reduced[:, 1], c=iris.target, cmap='viridis', edgecolor='k')
        ...
        scatter = ax.scatter(pca_reduced[:, 0], pca_reduced[:, 1], pca_reduced[:, 2], 
